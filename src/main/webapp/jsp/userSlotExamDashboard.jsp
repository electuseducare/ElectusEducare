            ...
            <div class="col-lg-12">
               <div class="panel panel-default">
                  ...
                  <div class="panel-body">
                    <%--  <form:form modelAttribute="userstartexam" id="testform" method="POST" action="#" > --%>
                        <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example" style="font-size: 12px;">
                           <thead>
                              <tr>
                                 ...
                                 <th>EXPERIY <span class="glyphicon glyphicon-time"></span></th>
                           ...
                           <tbody>
                              <c:forEach items="${questionlist}" var="displayexam" varStatus="packageloop">
                                 <tr>
                                    <td style="font-weight: bold;"> ${displayexam.examname}</td>
                                    <td style="font-weight: bold;"> ${displayexam.exam_type}</td>
                                    <td style="font-weight: bold;">
                                       <c:forEach items="${displayexam.sublist}" var="sublist"> ${sublist}
                                       </c:forEach>
                                    </td>
                                    <td style="font-weight: bold;"> ${displayexam.enddate}</td>
                                    ...
                                    <c:set var="exenddate" value="${displayexam.examenddate}"/>
                                    <c:set var="exendtime" value="${displayexam.examendtime}"/>
                                    
                                    <%
                                       DateFormat dateformat=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                                       Date currentdate = new Date();
                                       
                                       String testexpiredate=(String)pageContext.getAttribute("slotdate");
                                       String exenddate=(String)pageContext.getAttribute("exenddate");
                                       String exendtime=(String)pageContext.getAttribute("exendtime");
                                       
                                       if(exenddate==null && exendtime==null){
                                    	   exenddate="0000-00-00";
                                    	   exendtime="00:00:00";
                                          }
                                       
                                       String exdate = exenddate+" "+exendtime;
                                       String examExpDate=testexpiredate+" "+"23:59:59";
                                       	 
                                         Date parseExamExpDate = dateformat.parse(examExpDate);
                                         String  formatexamExpDateTime = dateformat.format(parseExamExpDate);
                                         
                                         Date exxmdate = dateformat.parse(exdate);
                                         String  exxmdate1 = dateformat.format(exxmdate);
                                         
                                          String currentDateTime=dateformat.format(currentdate);
                                          
                                             pageContext.setAttribute("currentDateTime", currentDateTime); 
                                             pageContext.setAttribute("formatexamExpDateTime", formatexamExpDateTime); 
                                             pageContext.setAttribute("exxmdate1", exxmdate1); 
                                    ...
                                    <c:set var="istestexpired" value="false"/>
                                    <c:set var="currentDateTime" value="${currentDateTime}"> </c:set>
                                    <c:set var="formatexamExpDateTime" value="${formatexamExpDateTime}"> </c:set>
                                    <td>
                                       <c:choose>
                                          <c:when test="${(displayexam.exam_status == 'NotStarted')&&(currentDateTime<=formatexamExpDateTime)}">
                                             <c:if test="${displayexam.patterntypeid==1}">
                                             <a href="load-instructions?exam=${displayexam.examname},start,${myVar}"><button id="start_${packageloop.index}" class="btn-primary" 
                                             ...
                                             <c:if test="${displayexam.patterntypeid!=1}">
                                             <a href="load-ExamForm1?exam=${displayexam.examname},start,${myVar}"><button id="start_${packageloop.index}" class="btn-primary" 
                                                style="padding: 5px 10px;
                                                font-size: 15px;
                                                text-align: center;
                                                cursor: pointer;
                                          ...
                                          <c:when test="${(displayexam.exam_status == 'start')&&(currentDateTime>=exxmdate1)}">
                                             <label style="color: red;">Exam Not Finished</label>
                                          </c:when>
                                          <c:when test="${(displayexam.exam_status == 'start')&&(currentDateTime<=exxmdate1)}">
                                             <c:if test="${displayexam.patterntypeid==1}">
                                             <a href="load-instructions?exam=${displayexam.examname},restart,${myVar}">
                                          ...
                                          <c:if test="${displayexam.patterntypeid!=1}">
                                           <a href="load-ExamForm1?exam=${displayexam.examname},restart,${myVar}">
                                             <button class="btn-primary"  
                                                style="padding: 5px 10px;
                                                font-size: 15px;
                                                text-align: center;
                                          ...
                                          <c:when test="${(displayexam.exam_status == 'Finish')}">
                                          ...
                                          <div class="modal-content">
                                             ...
                                             <div class="modal-body">
                                                ...
                                                <div id="gatepattern_${packageloop.index}" style="display: none">
                                                   <p>1.Total duration of the examination is <label>${displayexam.endtime} HOURS</label>.</p>
