	...
	virtualpaginate.prototype.navigate=function(keyword){
		...
		for (var p=0; p<this.paginatediv.length; p++){ //loop through all pagination DIVs
			if (this.flatviewpresent)
				this.flatviewlinks[p][prevlinkindex].className="selected11" //"Unhighlight" previous page (before this.currentpage increments)
	...
	virtualpaginate.prototype.paginate_output_flatview=function(flatviewcontainer, divpos, anchortext){
		...
		for (var i=0; i<this.pagecount; i++){
			if (typeof anchortext[i]!="undefined"){ //if custom anchor text for this link exists
				flatviewhtml+='<a href="#flatview" id="span_'+(pgk)+'" rel="'+i+'" >'+anchortext[i]+'</a> '; //build pagination link using custom anchor text
			}
			else{
				
				flatviewhtml+='<a href="#flatview" id="span_'+(pgk)+'" rel="'+i+'" >'+(i+1)+'</a> '; //build  pagination link using auto incremented sequential number instead
