            	...
            	$.each(data,function(i,val){
            		...
            		$.each(val,function(key,value1){
                		...
                		subjectnmames = value1;
                        	...
                        	document.getElementById("subjecttypediv").innerHTML+='<input type="checkbox" name="subjectname" id="'+value1+'" value="'+value1+'" onclick="populateinputBoxes(this.id);getsetTopicstypeAjax(this.id); uncheckquestiontypes_subjectunchecked(this.id);"/>'+""+content+"("+value1+")";
...
function populateinputBoxes(subjids){
	...
	if(document.getElementById(subjids).checked == true){
		...
		$("#noofqns_"+subjids).each(function() {
		    ...
		    $('#questionsFrom_'+subjids).show();
		    $('#questionsTo_'+subjids).show();
	...
	if(document.getElementById(subjids).checked == false){
		...
		$("#noofqns_"+subjids).each(function() {
		    ...
		    $('#questionsFrom_'+subjids).hide();
		    $('#questionsTo_'+subjids).hide();
		    $('input.check21').prop('checked',this.checked);
...
function qusperqustype(qustypeid,qtid,qmarksclass, qtypename,negativmarksdiv,qnsfromdiv,qnstodiv){
	var mrksperqntype = "";
	var subjectcnt = 0;
	var subjectids = "";
	var content="";
    ...
    $("[name='subjectname']").each( function() {
    	...
    	if (document.getElementById(subjectids).checked){
    		if( document.getElementById(qtid).checked==true){
    			//alert(qtid);
    	         qnscontent+='<table><tr><td><input type="text" name="'+subjectids+"_"+qustypeid+'" class="qntypeqnsclass" id="nofoqus" placeholder="No.of que per Ques type in '+subjectids+'" style="width:620px;"  onkeypress="return event.charCode >= 48 && event.charCode <= 57" maxlength="3" minlength="1" onkeyup="dataBaseValidations(this.value,'+subjectids+','+qustypeid+');"/>';
    	         qnsFrom+='<select name="'+subjectids+"questionsfrom"+qustypeid+'" id="'+subjectids+"questionsfrom"+qustypeid+'" class="qntypemarksclass" style=" width: 100px; border-radius: 4px;padding: 4px 7px; margin-left:20px;" onchange="addQuestionsToValues(this.name,this.value)">';
    	         qnsFrom+='<option value=0>--Select--</option>';
    	         for (i = 1; i <= 200; i++) { 
             		qnsFrom+='<option value='+i+'>'+i+'</option>';
             	}
             	qnsFrom+='</select>';
             	qnsTo+='<input type="text" name="'+subjectids+"questionsto"+qustypeid+'" id="'+subjectids+"questionsto"+qustypeid+'"  class="qntypemarksclass" style=" width: 100px; border-radius: 4px;padding: 3px 5px;" readonly> </td></tr></table>';
             
             	
                  
    	         mrksperqntype+='<table><tr><td><input type="text" name="'+subjectids+"_marks_"+qustypeid+'" class="qntypemarksclass" id="nofoqusmarks" placeholder=" Marks assigned per '+subjectids+' per '+qtypename+'" style="width:620px;" onkeypress="return event.charCode >= 48 && event.charCode <= 57" maxlength="1" minlength="1"/></table></td></tr>';
        	     netativecontent+='<table><tr><td><input type="text" name="'+subjectids+"_negmarks_"+qustypeid+'" class="qntypemarksclass" id="nofoqusnegmarks" placeholder=" Negative Marks per '+subjectids+' per '+qtypename+'" style="width:620px;" onkeypress="return event.charCode >= 48 && event.charCode <= 57" maxlength="1" minlength="1"/></table></td></tr>';
    ...
    document.getElementById(qnsfromdiv).innerHTML=qnsFrom;
    document.getElementById(qnstodiv).innerHTML=qnsTo;
