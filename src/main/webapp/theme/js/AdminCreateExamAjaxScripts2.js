...
function validExamnameexists(examname){
    ...
    if(isValid==false){
    	examnamevalue='';
    	 document.getElementById("examerror").innerHTML='Exam name already exists. Please try with other name';
    }
    if(isValid!=false){
...
var getsubjectsFromClasses = function(classid){
	...
	document.getElementById('currenttopics').style.display = 'block'; 
	  document.getElementById('currentsubtopics').style.display = 'block'; 
	var classid = document.getElementById(classid).value;
    $.ajax({
...
function populateinputBoxes(subjids){
	...
	if(document.getElementById(subjids).checked == true){
		  ...
		  $( "#topicsdiv1" ).remove();
		  $( "#prevtopicsdiv1" ).remove();
		  $( "#subtopicsdiv1" ).remove();
		  $( "#prevsubtopicsdiv1" ).remove();
		  $("span").remove(".previousqnsdata");
		  $("span").remove(".previousmarksdata");
		  $("span").remove(".previousnegmarksdata");
...
function qusperqustype(qustypeid,qtid,qmarksclass, qtypename){
    ...
    $("[name='subjectname']").each( function() {
    	...
    	if (document.getElementById(subjectids).checked){
    		if( document.getElementById(qtid).checked==true){
    			//alert(qtid);
    	         qnscontent+='<table><tr><td><input type="text" name="'+subjectids+"_"+qustypeid+'" class="qntypeqnsclass" id="nofoqus" placeholder="No.of que per Ques type in '+subjectids+'" style="width:300px;"  onkeypress="return event.charCode >= 48 && event.charCode <= 57" maxlength="3" minlength="1"/></table></td></tr>';
    	         if (document.getElementById('yesCheckmarks').checked==false){ 
        	         ...
        	         netativecontent+='<table><tr><td><input type="text" name="'+subjectids+"_negmarks_"+qustypeid+'" class="qntypemarksclass" id="nofoqusnegmarks" placeholder="Negative marks per '+subjectids+' per '+qtypename+'" style="width:300px;" onkeypress="return event.charCode >= 48 && event.charCode <= 57" maxlength="1" minlength="1"/></table></td></tr>';  
    	         }
    ...
    document.getElementById(qtypename).innerHTML=netativecontent;
