...
@Controller
public class AdminCreateExamWithSelectedQuestionsController {
	...
	@RequestMapping("load-createSelectedQuestionsExam")
	public String selectedQuestionsExam(Model model, HttpSession ses, AdminCategory ad,HttpServletRequest req) {
		...
		return "AdminCreateSelectedQuestionsExam";

	}


	@RequestMapping("load-GetFilenamesforSubjects1")
	public @ResponseBody List<AdminCategory> getFilenamesinSubjs1(Model model, AdminFormSetExamPojo adf,
			AdminCategory ad, HttpServletRequest req, HttpSession ses) {

		String start = "Entry of getFilenamesinSubjs1 method....";
		String end = "End of getFilenamesinSubjs1 method....";
		...
		int etypeid = (new Integer(examtypeid)).intValue();
		String totalvalue = req.getParameter("totalval");
		String totalval = totalvalue.substring(0, totalvalue.length() - 1);
		...
		for (int a = 0; a < splitval.length; a++) {
			...
			String[] topicarr = subval.split("-");
			String subid = topicarr[0];
			String topicval = topicarr[1];
			String[] stopicarr = topicval.split("_");
			String topicid = stopicarr[0];
			String subtopicid = stopicarr[1];

			/**** get all topics ***/
			if (topicid.contains("All Topics")) {
				topicid = adminuserservice.getAllTopicIds(subid, etypeid);
			}

			/*** get all sub topics ***/
			if (subtopicid.contains("All STopics")) {
				subtopicid = adminuserservice.getAllSubTopicIds(subid, etypeid);
			}

			filenames = adminuserservice.getFilenamesforSubjects1(subid, examtypeid, topicid, subtopicid);
	...
	@RequestMapping(value = "/insert-SelectedQuestionsForm", method = RequestMethod.POST)
	public String insertSelectedgivenExam(Model model, AdminCategory ad, HttpServletRequest req, HttpSession ses)
		...
		logger.info(end);
		return "redirect:/selectedQusCreatedExam";

	}

	@RequestMapping(value = "/selectedQusCreatedExam", method = RequestMethod.GET)
	public String finalCreateExamPage(Model model, HttpServletRequest req, HttpSession ses) {
		...
		return "AdminSuccessCreateExam1";
	}

	@RequestMapping(value = "/load-getQuestionTypesFromFilenames")
	public @ResponseBody List<AdminQtypesFromSlectedFileNamesModel> getQuestionsTypesForSelectedExam(Model model,
			AdminQtypesFromSlectedFileNamesModel aq, HttpServletRequest req, HttpSession ses) {

		String start = "Entry of getQuestionsTypesForSelectedExam method....";
		String end = "End of getQuestionsTypesForSelectedExam method....";

		logger.info(start);
		model.addAttribute(selexamval, aq);

		String studentid = (String) ses.getAttribute(sidval);
		lc.getpermissionsforloggedusers(model, studentid);

		String filenames = req.getParameter("filenamegroup");
		List<AdminQtypesFromSlectedFileNamesModel> qtypelist = adminuserservice.getQtypesFromFileName(filenames);
		logger.info(end);
		return qtypelist;

	}

	@RequestMapping(value = "/load-getQuestionsBaesedonOnfilenames")
	public @ResponseBody List<AdminCategory> getQuestionsTypesForSelectedExam(Model model, HttpServletRequest req,
			HttpSession ses, AdminCategory aq) {
		...
		List<AdminCategory> questionslist = adminuserservice.getQuestionsFromFileName(filenames);
