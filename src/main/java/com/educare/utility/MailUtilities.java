...
public class MailUtilities 
{
	...
	private static String EMAIL_FROM_PASSWORD = "";
	...
	public static void sendMail(MailPojo mp) throws MessagingException 
	{
		boolean debug = false;
		int toRecipientsCount = mp.getToRecipients().size(); // get TO recipients count
		int ccRecipientsCount = mp.getCCRecipients().size(); // get CC recipients count
		int bccRecipientsCount = mp.getBCCRecipients().size(); // get BCC recipients count
		int attachedFilesCount = mp.getMailAttachments().size(); // get attached files count
		
		//Validation
		String subject = mp.getMailSubject();
		String message = mp.getMsgContent();
		EMAIL_FROM=mp.getMailid();
		EMAIL_FROM_PASSWORD=mp.getMailpwd();
		...
		{
			toAddresses[i] = new InternetAddress(mp.getToRecipients().get(i));
		}
		...
		if(ccRecipientsCount>0)
		{
			...
			{
				ccAddresses[i] = new InternetAddress(mp.getCCRecipients().get(i));
			}
		...
		if(bccRecipientsCount>0)
		{
			...
			{
				bccAddresses[i] = new InternetAddress(mp.getBCCRecipients().get(i));
			}
		...
		if(attachedFilesCount>0)
		{
	         ...
	         messageBodyPart.setText(mp.getMsgContent());
	         Multipart multipart = new MimeMultipart();
	         //add the message body to the MIME message.
	         multipart.addBodyPart(messageBodyPart);
	         // add any file attachments to the message.
	         addAtachments(mp.getMailAttachments(), multipart);
