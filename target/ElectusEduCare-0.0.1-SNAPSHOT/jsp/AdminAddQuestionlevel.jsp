   ...
   <head>
      ...
      <script type="text/javascript">
         function fieldvalidation(){
         	...
         	var examname = document.getElementById('questionlevel').value;
         	var len =examname.replace(/\s+$/, '');
             if(len==''){
             	alert('Question level should not be empty');
             	return false;
             }
   ...
   <body>
      ...
      <div style="width:70%; float:left; background-color: #ffffff">
         <ul class="nav nav-tabs">
            <li class="active" ><a href="#" >ADD QUESTION LEVEL</a></li>
         ...
         <div class="panel-heading" style="background-color:#ccf2ff;color:black; font-weight: bold;">
            <a href="load-addquestionlevel">
            <img border="0" alt="questionlevel" src="${pageContext.request.contextPath}/theme/images/addicon.jpg" width="40" height="40" style="background-color:#ccf2ff">
            </a>
         </div>
         <div class="panel-body" id="mockTabs" style="background-color:#DFE2DB;">
            ...
            <c:if test="${smsg==null}" >
               <form:form  name="AddRoleform" method="post" action="process-questionlevelform"  modelAttribute="admincategory" >
                  <label style="color:black;font-weight: bold;">Question Level:</label>
                  <form:input path="questionlevel" id="questionlevel" class="form-control" placeholder="Enter Question Level" required="required"  maxlength="60" autocomplete="off"/>
                  <div class="row"></div>
                  <center><button class="btn-primary" style="background-color: #00a3cc;color:white;width:20%;align-text:center;" onclick="return fieldvalidation();">ADD QUESTION LEVEL</button></center>
               </form:form>
            </c:if>
         </div>
      </div>
      ...
      <script>
         var input = document.getElementById('questionlevel');
