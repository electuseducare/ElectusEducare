...
document.write('<style type="text/css">' //write out CSS for class ".hidepeice" that hides pieces of contents within pages
	...
	virtualpaginate.prototype.navigate=function(keyword){
		...
		for (var p=0; p<this.paginatediv.length; p++){ //loop through all pagination DIVs
			if (this.flatviewpresent)
				this.flatviewlinks[p][prevlinkindex].className="selected1" //"Unhighlight" previous page (before this.currentpage increments)
	...
	virtualpaginate.prototype.paginate_output_flatview=function(flatviewcontainer, divpos, anchortext){
		...
		for (var i=0; i<this.pagecount; i++){
			if (typeof anchortext[i]!="undefined"){ //if custom anchor text for this link exists
				flatviewhtml+='<div id="qtd'+pgk+'"><a href="#flatview" id="button_'+(pgk)+'" rel="'+i+'" ><span title="Not Answered" class="not_visited auditlog" id=span_'+pgk+' onclick=visitednumber(this.id)>'+pgk+'</span></a></div>'; //build pagination link using custom anchor text
			}
			else{
				
				flatviewhtml+='<div id="qtd'+pgk+'"><a href="#flatview" id="button_'+(pgk)+'" rel="'+i+'" ><span title="Not Answered" class="not_visited auditlog" id=span_'+pgk+' onclick=visitednumber(this.id)>'+pgk+'</span></a></div>'; //build  pagination link using auto incremented sequential number instead
	...
	virtualpaginate.prototype.paginate_build_flatview=function(flatviewcontainer, divpos, anchortext){
		...
		this.flatviewlinks[divpos][this.currentpage].className="selectedclass" //"Highlight" current flatview link
...
function visitednumber(qnoval){
	 if($("#"+qnoval).hasClass( "not_visited auditlog")){
		 $("#"+qnoval).removeClass().addClass('not_answered auditlog');
     }
}
</script>
